...
export class EditGroupModalComponent {
  ...
  addMembers(): void {
    ...
    if (this.newmemberToAdd.length > 0) {
      this.dataService
        .addMembersToGroup(this.newmemberToAdd, this.groupId)
        .subscribe(
          (response) => {
            // alert(`${this.newmemberToAdd.length} member(s) added successfully!`);
            this.alertService.success(
              `${this.newmemberToAdd.length} member(s) added successfully!`,
              {
                id: 'alert-1',
                autoClose: true,
              }
          ...
          (error) => {
            ...
            this.alertService.error(
              `Failed to add new members. Please try again.`,
              {
                id: 'alert-1',
                autoClose: true,
              }
      ...
      this.alertService.warn('Please add new members first or unknown error', {
        id: 'alert-1',
        autoClose: true,
      });
    }
